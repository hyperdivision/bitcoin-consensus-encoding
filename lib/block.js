...
const transaction = require('./tx.js')
...
function decode (buf, offset) {
  ...
  for (let i = 0; i < block.txnCount; i++) {
    const tx = transaction.decode(buf, offset)
    offset += transaction.decode.bytes
...
fs.readFile('../block.raw', function (err, data) {
  if (err) throw err
  const hexBuf = Buffer.from(data.toString(), 'hex')
  const block = decode(hexBuf)
  console.log(block)
})
